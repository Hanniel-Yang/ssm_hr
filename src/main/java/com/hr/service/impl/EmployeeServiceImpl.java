...
public class EmployeeServiceImpl  extends ServiceImpl<EmployeeMapper, Employee> implements EmployeeService {
     ...
     */
    @Override
    public void deleteByDepartmentNumber(Integer departmentNumber) {
        //先查询再删除，否则NullPointerException
        Employee employee = employeeMapper.selectByDepartmentNumber(departmentNumber);
        //删除在职员工记录
       employeeMapper.deleteByDepartmentNumber(departmentNumber);
        //将员工档案表中的状态改为离职
        History history = historyMapper.selectByNumber(employee.getEmployeeNumber());
        history.setStatus("离职");
        historyMapper.updateById(history);
